...
(function ($, Drupal, window, document, undefined) {
    ...
    Drupal.behaviors.yogatuneup = {
        attach: function (context, settings) {
            ...
            // Dropdown nav fix
            if ($('html').hasClass('touch')) {
                $('.menu.nav > li.dropdown > a, .menu.nav > li.dropdown > span').click(function (e) {
                    var isOpen = $(this).hasClass('up-drop');
                    if (!isOpen) {
                        e.preventDefault();
                    }
                    $('.up-drop').removeClass('up-drop');
                    if ($(window).width() < 992) {
                        $(this).siblings('ul.dropdown-menu').toggleClass('open-mobile');
                    }
                    if (!isOpen) {
                        $(this).toggleClass('up-drop');
                    }
                });
                $('.dropdown-menu .leaf').tap(function () {
        ...
        attach: function (context, settings) {
            function bgImage() {
                if (settings.hasOwnProperty('homepage') && settings.homepage.body_bg !== "") {
                    if ($(window).width() < 551) {
                        $('.field-name-body').css('background-image', 'none');
                    }
                    else if ($(window).width() > 550 && $(window).width() < 992) {
                        $('.field-name-body').css('background-image', 'url("' + settings.homepage.body_bg + '")').css('background-repeat', 'no-repeat').css('background-size', 'contain').css('background-position', '5px 275px').css('min-height', '480px');
                    }
                    else {
                        $('.field-name-body').css('background-image', 'url("' + settings.homepage.body_bg + '")').css('background-repeat', 'no-repeat').css('background-size', 'contain').css('background-position', '60px 110px').css('min-height', '420px');
                    }
            ...
            $('.blog-item .blog-title, .blog-item .blog-description').dotdotdot();

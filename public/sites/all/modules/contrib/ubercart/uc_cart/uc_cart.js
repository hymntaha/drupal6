...
Drupal.behaviors.ucCart = {
  attach: function(context, settings) {
    ...
    jQuery('form#uc-cart-checkout-review-form input#edit-submit:not(.ucSubmitOrderThrobber-processed)', context).addClass('ucSubmitOrderThrobber-processed').click(function() {
      jQuery(this).clone().insertAfter(this).attr('disabled', 'disabled').after('<div class="ajax-progress" style="display:inline-block;"><div class="throbber">&nbsp;</div></div>').end().hide();
      jQuery('#uc-cart-checkout-review-form #edit-back').attr('disabled', 'disabled');
      jQuery.blockUI({
        message: '<h2>Processing Order. Please Wait...</h2>',
        timeout: 0
      });
    });
  }

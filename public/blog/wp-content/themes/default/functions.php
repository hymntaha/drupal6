...
////////////////////////////////////////////////////////////////////
function ytu_theme_stylesheets() {
...
add_action('wp_enqueue_scripts', 'ytu_theme_stylesheets');

function ytu_theme_scripts() {

   // Place any new global script enqueues here
}

add_action('wp_enqueue_scripts', 'ytu_theme_scripts');

function ytu_page_start(){
   global $page_start_time;
   $page_start_time = microtime(true);
   ob_start();
}

function ytu_page_end(){
    global $page_start_time;
    ...
    $chrome = ytu_retrieve_chrome();
    $duration = microtime(true)-$start_time;

    $contents = str_replace("{{WP_CONTENTS_REPLACE}}", $contents, $chrome);

    $contents = str_replace("</head>", $header."</head>", $contents);
    $contents = str_replace("</body>", $footer."</body>", $contents);
    echo $contents;
}


function ytu_retrieve_chrome(){
    $headers = array();
    ...
    $url = "http".($_SERVER['HTTPS'] ? "s" : "")."://".$_SERVER['HTTP_HOST']."/blogs/chrome";
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
    curl_setopt($ch, CURLOPT_COOKIE, $_SERVER['HTTP_COOKIE']);
    $chrome = curl_exec($ch);

    if(!strpos($chrome, "{{WP_CONTENTS_REPLACE}}")){
        $chrome = false;
    }

    if($chrome === false){
        $chrome = "<html><head></head><body>{{WP_CONTENTS_REPLACE}}</body></html>";
    }
